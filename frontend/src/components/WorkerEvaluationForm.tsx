...
interface Category {
  value: 'quality' | 'punctuality' | 'communication' | 'overall';
  label: string;
  icon: string;
}

export default function WorkerEvaluationForm({ 
  workerId, 
  workOrderId, 
  onSuccess, 
  onCancel 
  ...
  const checkExistingEvaluation = async (evaluatorId: string): Promise<void> => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const evaluation = {
        workerId,
        evaluatorId: userId,
        evaluatorType: 'seller' as const, // or 'customer' based on context
